...
export class Web3Service {
  ...
  public async web3login() {
    MEDICAREJSON = require('../../../../build/contracts/Medicare.json')
    return new Promise(async (resolve, reject) => {
      ...
      if (window.ethereum || window.web3) {
        ...
        if (typeof MEDICAREJSON.networks[Net] === 'undefined') {
        ...
        this.AccountSubscription = this.RefreshedAccount.subscribe(async () => {
          ...
          if (
            ...
            const MedicareInstance = new window.web3.eth.Contract(
              MEDICAREJSON.abi,
              MEDICAREJSON.networks[Network].address
            )
            this.Web3Details$.next({
              account: Account,
              network: Network,
              medicare: MedicareInstance.methods
            })
          }
